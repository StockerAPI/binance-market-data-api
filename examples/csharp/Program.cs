...
class Program
{
    ...
    private const string Market = "BINANCE";
    private const string Exchange = "BINANCE";
    private const string Symbol = "BINANCE:BTCUSDT";
    ...
    {
        using var client = new HttpClient();
        var url = $"{HttpBase}/api/exchange?market={Uri.EscapeDataString(Market)}&venue={Uri.EscapeDataString(Exchange)}&tickers=BTCUSDT,ETHUSDT&token={Uri.EscapeDataString(Token)}";
    ...
    static async Task StreamRealtime()
    {
        ...
        var subscribe = new
        {
            ...
            market = Market,
            exchange = Exchange,
            symbols = new[] { "BINANCE:BTCUSDT", "BINANCE:ETHUSDT" },
            replay = "last"
        };
