...
async function callExchange() {
  const url = `${httpBase}/api/exchange?market=${encodeURIComponent(market)}&venue=${encodeURIComponent(exchange)}&tickers=BTCUSDT,ETHUSDT&token=${encodeURIComponent(token)}`;
  const res = await fetch(url);
  const text = await res.text();
